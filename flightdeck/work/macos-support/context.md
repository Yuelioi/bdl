...
- Credentials must use the operating system secure store; cookies must never fall back to plain files, SQLite, logs, or frontend state.
...
- Platform-correct Keychain, window controls, paths, FFmpeg execution, file reveal/open behavior, and updater artifact metadata.
