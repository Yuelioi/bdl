...
- [x] Persist selected quality, audio, codec, container, final path, download time, and error summary for completed records.
...
- [x] Add diagnostics export from Settings or Transfer inspector.
- [x] Extend redaction coverage for cookies, authorization headers, signed URLs, and large response bodies.
- [x] Add action-specific errors for unwritable save directory, missing ffmpeg, login expired, private resource, and parse recognition failure.
...
- [x] Verify account state at startup and emit `account://updated`.
...
- [x] Rewrite Task 15 around current product decisions: no separate History page, completed records live in Transfer.
- [x] Mark older completed Task 1-3 plan checkboxes consistently or replace them with an audit summary to avoid false "unfinished" signals.
