...
<template>
  <span v-if="sources.length === 0" class="inline-flex h-7 items-center gap-2 text-xs font-semibold text-[var(--color-muted)]">
    <span class="size-1.5 rounded-full bg-[var(--color-dimmed)]" aria-hidden="true"></span>
    等待解析
  </span>

  <span
    v-else-if="sources.length === 1"
    class="inline-flex h-7 items-center gap-2 text-xs text-[var(--color-muted)]"
  >
    <UIcon class="size-3.5 text-[var(--color-success)]" name="i-tabler-circle-check" aria-hidden="true" />
    <strong class="text-[var(--color-text)]">已解析 1 个来源</strong>
  </span>
  ...
  >
    <UiButton class="max-w-80" variant="secondary" size="compact">
      ...
      <span class="min-w-0 truncate text-[var(--color-muted)]">{{ activeSource?.source.title }}</span>
      <UIcon class="size-3.5 shrink-0 text-[var(--color-muted)]" name="i-tabler-chevron-down" aria-hidden="true" />
    </UiButton>
  </UDropdownMenu>
</template>
